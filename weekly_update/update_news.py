...
if dt.date.today().weekday() == 6:
    ...
    for index, (security_id, ticker) in enumerate(tickers):
        ...
        for item in sa_news:
            ...
            news_type_id = cur.execute("SELECT id FROM news_type WHERE name = ?", (item["type"],)).fetchone()[0]
            cur.execute(
                """
            ...
            news_id = cur.execute("SELECT id FROM security_news WHERE source_id = ? AND type_id = ? AND url = ?", (source_id, news_type_id, item["url"])).fetchone()[0]
        ...
        for item in nasdaq_news:
            ...
            source_id = cur.execute("SELECT id FROM news_source WHERE name = ?", (item["source"],)).fetchone()[0]
            news_type_id = cur.execute("SELECT id FROM news_type WHERE name IS NULL").fetchone()[0]
            cur.execute(
                """
            ...
            for related_ticker in item["related_tickers"]:
                related_id = cur.execute("SELECT id FROM securities WHERE ticker = ?", (related_ticker,)).fetchone()
