...
class NPORTInsert:
    ...
    def insert_filer_information(self) -> None:
        self.cik = self.filing.filer["cik"]
        ...
        # if there is an entry with the LEI but without a cik
        print(lei, self.cik)
        if db.cur.execute("SELECT lei FROM entity WHERE lei = ? AND cik IS NULL", (lei,)).fetchone() is not None:
            self.db.cur.execute("DELETE FROM entity WHERE cik = ?", (self.cik,))
            self.db.cur.execute(
                """
                UPDATE entity SET cik = ?, old_name = ?, sic_industry_id = ?, fiscal_year_end = ?, irs_number = ?
                WHERE lei = ?
                """,
                (self.cik, former_name, sic_industry_id, fiscal_year_end, self.filing.filer["irs_number"], lei)
            )
        else:
            self.db.cur.execute("INSERT OR IGNORE INTO entity (cik, added) VALUES (?, ?)", (self.cik, self.ts_today))
            try:
                self.db.cur.execute(
                    """
                    UPDATE entity SET lei = ?, old_name = ?, sic_industry_id = ?, fiscal_year_end = ?, irs_number = ?
                    WHERE cik = ?
                    """,
                    (lei, former_name, sic_industry_id, fiscal_year_end, self.filing.filer["irs_number"], self.cik)
                )
            except IntegrityError:
                self.db.cur.execute("INSERT OR IGNORE INTO entity (lei, added) VALUES (?, ?)", (lei, self.ts_today))

        self.entity_id = self.db.cur.execute("SELECT entity_id FROM entity WHERE cik = ?", (self.cik,)).fetchone()[0]
        ...
        if business is not None:
            ...
            self.db.cur.execute("INSERT OR IGNORE INTO sec_business_address (cik) VALUES (?)", (self.cik,))
            self.db.cur.execute(
                """
                UPDATE sec_business_address SET street1 = ?, street2 = ?, city_id = ?, state_id = ?, zip = ?, phone = ?
                WHERE cik = ?
                """,
                (business["street1"], business["street2"], city_id, state_id, business["zip"], business["phone"], self.cik)
            )
        ...
        if mail is not None:
            ...
            self.db.cur.execute("INSERT OR IGNORE INTO sec_mail_address (cik) VALUES (?)", (self.cik,))
            self.db.cur.execute(
                """
                UPDATE sec_mail_address SET street1 = ?, street2 = ?, city_id = ?, state_id = ?, zip = ?
                WHERE cik = ?
                """,
                (mail["street1"], mail["street2"], city_id, state_id, mail["zip"], self.cik)
            )
    ...
    def insert_portfolio_data(self) -> None:
        ...
        for index, item in enumerate(portfolio):
            ...
            if issuer["lei"] is not None:
                self.db.cur.execute("INSERT OR IGNORE INTO entity (lei, added) VALUES (?, ?)", (issuer["lei"], self.ts_today))
                self.db.cur.execute("UPDATE entity SET name = ?, type_id = ? WHERE lei = ?", (issuer["name"], issuer_type_id, issuer["lei"]))
    ...
    def insert_series_class_information(self) -> None:
        ...
        if series["cik"] is not None and series["lei"] is not None:
            self.db.cur.execute("INSERT OR IGNORE INTO sec_mf_series (cik, entity_cik, added) VALUES (?, ?, ?)", (series["cik"], self.cik, self.ts_today))
            try:
            ...
            self.db.cur.execute("INSERT OR IGNORE INTO sec_mf_series (lei, entity_cik, added) VALUES (?, ?, ?)", (series["lei"], self.cik, self.ts_today))
        ...
        if classes is not None:
            for class_ in classes:
                ...
                self.db.cur.execute("INSERT OR IGNORE INTO sec_mf_class (class_id, series_id, cik) VALUES (?, ?, ?)", (security_id, self.series_id, class_["cik"]))
        ...
        if self.filing.general_information["classes"] is not None:
            ...
            for class_cik, data in self.filing.return_information["class_returns"].items():
                if class_cik not in cik_ticker_match or cik_ticker_match[class_cik] is None:
                    continue
                security_id = self.db.cur.execute("SELECT class_id FROM sec_mf_class WHERE cik = ?", (class_cik,)).fetchone()[0]
...
if __name__ == "__main__":
    ...
    with Database() as db:
        urls = db.cur.execute(
            """
            SELECT document_url FROM sec_filing
            WHERE
            cik = 1579982
            AND
            type_id = (SELECT type_id FROM sec_form_type WHERE name = "NPORT-P")
            """
        ).fetchall()
        urls_parsed = []
        #urls = ["https://www.sec.gov/Archives/edgar/data/1424212/0001145549-22-079549.txt"]
        urls = db.cur.execute("SELECT document_url FROM sec_filing WHERE type_id = (SELECT type_id FROM sec_form_type WHERE name = 'NPORT-P')").fetchall()
        urls = [item[0] for item in urls]
        print(len(urls))
        #urls = ["https://www.sec.gov/Archives/edgar/data/1168164/000175272421043865/0001752724-21-043865.txt"]
        #urls = ["https://www.sec.gov/Archives/edgar/data/1100663/000175272423123221/0001752724-23-123221.txt"]
        #for index, url in enumerate(urls):
        #urls = ["https://www.sec.gov/Archives/edgar/data/1402472/0001398344-22-022758.txt"]
        for index in range(3000):
            url = random.choice(urls)
            print(f"{index+1:>3} of 3000: {url}")
            file = FilingNPORT(url=url)
            NPORTInsert(file, db).insert_all()
            urls_parsed.append(url)
        db.cur.execute(f"UPDATE sec_filing SET parsed = True WHERE document_url IN {tuple(urls_parsed)}")
        """for url in urls:
            print(url)
            file = FilingNPORT(url=url)
            if file.general_information["classes"][0]["ticker"] == "ARKK":
                continue
            NPORTInsert(file, db).insert_all()"""