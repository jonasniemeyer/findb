...
for index, ticker in enumerate(tickers):
    ...
    for statement in dct.keys():
        for variable, data in dct[statement].items():
            var_id = cur.execute("SELECT variable_id from macrotrends_fundamental_variable WHERE name = ?", (variable,)).fetchone()[0]
            data = data.items()
            financial_data = []
    ...
    cur.execute("UPDATE company SET macrotrends_fundamentals_updated = ?, fiscal_year_end = ? WHERE security_id = ?", (timestamp_today, fiscal_year_end.month, security_id))
