...
def update_companies(db_connection):
    ...
    for (cik, ticker, name) in database_companies:
        ...
        if (cik, ticker) not in new_companies.keys():
            if cur.execute("SELECT discontinued FROM security WHERE cik = ? AND ticker = ?", (cik, ticker)).fetchone()[0] is None:
        ...
        # if the company in the database IS in the new companies dict but has a different name, the name should be updated
            ...
            cur.execute(
                "UPDATE security SET sec_name = ?, old_name = ? WHERE cik = ? AND ticker = ?",
    ...
    for (cik, ticker), name in new_companies.items():
        ...
        # if the new company is already in the database, set discontinued to NULL in case if was discontinued in the past
            ...
            cur.execute("UPDATE security SET discontinued = NULL WHERE cik = ? AND ticker = ?", (cik, ticker))
...
def update_mutualfunds(db_connection):
    ...
    database_entities = cur.execute("SELECT cik FROM sec_mutualfund_entity").fetchall()
    ...
    for cik in database_entities:
        if (
            ...
            and cur.execute("SELECT discontinued FROM sec_mutualfund_entity WHERE cik = ?", (cik,)).fetchone()[0] is None
        ):
            cur.execute("UPDATE sec_mutualfund_entity SET discontinued = ? WHERE cik = ?", (ts_today, cik))
    ...
    for cik in new_entity_ciks:
        if cik not in database_entities:
            cur.execute("INSERT INTO sec_mutualfund_entity (cik, added) VALUES (?, ?)", (cik, ts_today))
            print(f"New Mutual Fund Entity: {cik}")
        else:
            cur.execute("UPDATE sec_mutualfund_entity SET discontinued = NULL WHERE cik = ?", (cik,))
    con.commit()
    ...
    for series_cik, entity_cik in new_series:
        if series_cik not in database_series:
            entity_id = cur.execute("SELECT entity_id FROM sec_mutualfund_entity WHERE cik = ?", (entity_cik,)).fetchone()[0]
    ...
    for class_ in new_classes:
        ...
        if (class_cik, ticker) not in database_classes:
            ...
            cur.execute("UPDATE security SET discontinued = NULL WHERE cik = ? AND ticker = ?", (class_cik, ticker))
    con.commit()
